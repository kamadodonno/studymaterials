...
class FirebaseService {
    ...
    private let apiKey = "AIzaSyD2pLPYO0MHy606kxNFbbAQSniXlvRDk9E"
    ...
    private let authQueue = DispatchQueue(label: "com.pumaterial.authQueue")
    
    private init() {}
    
    // MARK: - Ensure Valid Firebase Auth Token
    private func ensureAuthenticated(completion: @escaping (String?) -> Void) {
        authQueue.async {
            if let token = self.cachedIdToken, let expiry = self.tokenExpiryDate, expiry > Date() {
                completion(token)
                return
            }
            
            guard let authUrl = URL(string: "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=\(self.apiKey)") else {
                completion(nil)
                return
            }
            ...
            let payload: [String: Any] = ["returnSecureToken": true]
            ...
            URLSession.shared.dataTask(with: request) { data, response, error in
                ...
                let expiresIn = (json["expiresIn"] as? String).flatMap { Double($0) } ?? 3600
                self.cachedIdToken = idToken
                self.tokenExpiryDate = Date().addingTimeInterval(expiresIn - 60)
                completion(idToken)
            }.resume()
        }
    }
    ...
    func fetchSubjects(completion: @escaping ([Subject]) -> Void) {
        ensureAuthenticated { [weak self] token in
            guard let self = self, let token = token else {
                DispatchQueue.main.async { completion([]) }
                return
            }
            
            guard let url = URL(string: "\(self.baseUrl)/subjects?pageSize=100") else {
                DispatchQueue.main.async { completion([]) }
                return
            }
    ...
    func fetchModules(subjectId: String, completion: @escaping ([Module]) -> Void) {
        ensureAuthenticated { [weak self] token in
            ...
            guard let url = URL(string: "\(self.baseUrl)/modules?pageSize=100") else {
                DispatchQueue.main.async { completion([]) }
                return
            }
    ...
    func fetchMaterials(subjectId: String, section: String, completion: @escaping ([Material]) -> Void) {
        ensureAuthenticated { [weak self] token in
            ...
            guard let url = URL(string: "\(self.baseUrl)/materials?pageSize=100") else {
                DispatchQueue.main.async { completion([]) }
                return
            }
    ...
    func fetchAnnouncements(section: String, completion: @escaping ([Announcement]) -> Void) {
        ensureAuthenticated { [weak self] token in
            ...
            guard let url = URL(string: "\(self.baseUrl)/announcements?pageSize=50") else {
                DispatchQueue.main.async { completion([]) }
                return
            }
    ...
    private func createFirestoreMaterialDoc(
        ...
        ensureAuthenticated { [weak self] token in
            guard let self = self, let token = token else {
                completion(.failure(NSError(domain: "FirebaseService", code: -4, userInfo: [NSLocalizedDescriptionKey: "Auth failed"])))
                return
            }
            
            guard let url = URL(string: "\(self.baseUrl)/materials") else {
